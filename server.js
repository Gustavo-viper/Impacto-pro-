...
const FINANCE_SMS_TO = process.env.FINANCE_SMS_TO || '';
...
async function ensureSchema() {
  ...
  await db`CREATE TABLE IF NOT EXISTS finance_otp_challenges (id BIGSERIAL PRIMARY KEY, code_hash TEXT NOT NULL, expires_at TIMESTAMPTZ NOT NULL, attempts INTEGER NOT NULL DEFAULT 0, consumed_at TIMESTAMPTZ, requested_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), request_ip TEXT)`;
  await db`CREATE INDEX IF NOT EXISTS idx_finance_otp_requested ON finance_otp_challenges (requested_at)`;
  await db`CREATE INDEX IF NOT EXISTS idx_finance_otp_active ON finance_otp_challenges (expires_at, consumed_at)`;
  await db`CREATE TABLE IF NOT EXISTS client_form_submissions (id BIGSERIAL PRIMARY KEY, token TEXT NOT NULL, name TEXT NOT NULL, cpf_cnpj TEXT, rg TEXT, phone TEXT, email TEXT, address TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), imported_at TIMESTAMPTZ)`;
...
function maskPhone(phone){
  ...
  if(!d) return 'número administrativo cadastrado';
  return d.length>=4 ? `final ${d.slice(-4)}` : 'número administrativo cadastrado';
}
async function sendFinanceSms(code){
  if(!FINANCE_SMS_TO || !TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN || !TWILIO_FROM_NUMBER){
    throw new Error('SMS do Financeiro não configurado no servidor. Configure FINANCE_SMS_TO, TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN e TWILIO_FROM_NUMBER no Render.');
  }
  const body = new URLSearchParams({
    To: FINANCE_SMS_TO,
    From: TWILIO_FROM_NUMBER,
    Body: `Impacto Pro: seu código de acesso ao Financeiro é ${code}. Válido por ${OTP_TTL_MINUTES} minutos. Não compartilhe este código.`
  });
  ...
  if(!response.ok){
    ...
    console.error('Twilio SMS error:',txt);
    throw new Error('O provedor de SMS recusou o envio. Verifique as credenciais e o número remetente no Render.');
  }
}

app.post('/api/finance/request-code', async (req,res)=>{
  try{
    await ensureSchema();
    const db=getSql();
    const ip=String(req.headers['x-forwarded-for']||req.socket.remoteAddress||'unknown').split(',')[0].trim();
    const recent=await db`SELECT COUNT(*)::int AS count FROM finance_otp_challenges WHERE requested_at > NOW() - INTERVAL '60 seconds' AND request_ip=${ip}`;
    if(Number(recent[0]?.count||0)>=1) return res.status(429).json({ok:false,error:'Aguarde 60 segundos antes de solicitar outro código.'});
    const hourly=await db`SELECT COUNT(*)::int AS count FROM finance_otp_challenges WHERE requested_at > NOW() - INTERVAL '1 hour' AND request_ip=${ip}`;
    if(Number(hourly[0]?.count||0)>=5) return res.status(429).json({ok:false,error:'Limite de solicitações atingido. Tente novamente mais tarde.'});
    const code=String(crypto.randomInt(0,1000000)).padStart(6,'0');
    const expiresAt=new Date(Date.now()+OTP_TTL_MINUTES*60*1000);
    await db`UPDATE finance_otp_challenges SET consumed_at=NOW() WHERE consumed_at IS NULL OR expires_at < NOW()`;
    await db`INSERT INTO finance_otp_challenges(code_hash,expires_at,request_ip) VALUES(${hashOtp(code)},${expiresAt},${ip})`;
    await sendFinanceSms(code);
    res.json({ok:true,destination:maskPhone(FINANCE_SMS_TO),expiresInSeconds:OTP_TTL_MINUTES*60});
  }catch(e){
    console.error(e);
    res.status(500).json({ok:false,error:e.message||'Não foi possível enviar o código por SMS.'});
  }
});

app.post('/api/finance/verify-code', async (req,res)=>{
  try{
    await ensureSchema();
    const code=String(req.body?.code||'').replace(/\D/g,'');
    if(!/^\d{6}$/.test(code)) return res.status(400).json({ok:false,error:'Código inválido.'});
    const db=getSql();
    const rows=await db`SELECT id,code_hash,expires_at,attempts FROM finance_otp_challenges WHERE consumed_at IS NULL ORDER BY requested_at DESC LIMIT 1`;
    const row=rows[0];
    if(!row) return res.status(401).json({ok:false,error:'Nenhum código ativo. Solicite um novo código.'});
    if(new Date(row.expires_at).getTime() < Date.now()) return res.status(401).json({ok:false,error:'O código expirou. Solicite um novo código.'});
    if(Number(row.attempts||0)>=5) return res.status(401).json({ok:false,error:'Número máximo de tentativas atingido. Solicite um novo código.'});
    const valid=crypto.timingSafeEqual(Buffer.from(hashOtp(code),'hex'),Buffer.from(String(row.code_hash),'hex'));
    if(!valid){
      await db`UPDATE finance_otp_challenges SET attempts=attempts+1 WHERE id=${row.id}`;
      return res.status(401).json({ok:false,error:'Código incorreto.'});
    }
    await db`UPDATE finance_otp_challenges SET consumed_at=NOW() WHERE id=${row.id}`;
    res.json({ok:true,authorized:true});
  }catch(e){
    console.error(e);
    res.status(500).json({ok:false,error:'Não foi possível validar o código.'});
  }
});
...
app.get('/api/health', async (req, res) => {
  try {
    await ensureSchema();
    res.json({ ok: true, database: 'neon', version: '62.0' });
  } catch (e) {
...
app.listen(PORT, () => console.log(`Impacto Pro V62 rodando na porta ${PORT}`));
